...
const App = () => {
  ...
  // Vercel Blob: Listeleme ve Okuma (CORS sorunlarını aşmak için proxy kullanımı)
  const blobGetLatest = async () => {
    try {
      const listUrl = `${BLOB_BASE_URL}?prefix=${BLOB_FILENAME}`;
      // CORS kısıtlamalarını aşmak için proxy kullanıyoruz
      const proxyUrl = `https://api.allorigins.win/raw?url=${encodeURIComponent(listUrl)}`;
      
      const listRes = await fetch(proxyUrl, {
        headers: { 
      ...
      if (listData?.blobs && listData.blobs.length > 0) {
        // En güncel blob'un içeriğini direkt URL'den çekiyoruz (Public read desteklenir)
      ...
      console.warn("Blob Okuma Hatası (Proxy deneniyor):", e);
      return null;
    }
  };

  // Vercel Blob: Veri Yazma (Doğrudan PUT)
  ...
  const syncNews = async () => {
    ...
    // 1. Blob'dan mevcut yedeği oku
    const cached = await blobGetLatest();
    
    if (cached) {
      setNews(cached.articles || []);
      setLastNewsFetch(cached.lastFetched || null);

      // 2. Zaman kontrolü: 4.8 saat geçmediyse API'yi çağırma
  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-900 font-sans pb-12">
      ...
      <div className="bg-white border-b border-slate-200 py-1 sticky top-0 z-50 overflow-hidden shadow-sm">
        <div className="flex animate-marquee whitespace-nowrap items-center gap-12 text-xs font-bold text-slate-600">
          ...
          {commodities.map(c => (
            <div key={c.symbol} className="flex items-center gap-2">
              <span className="text-slate-400">{c.name.split(' ')[0]}</span>
              <span className="text-blue-600 font-mono">${formatMoney(c.price)}</span>
              <span className={c.change >= 0 ? 'text-emerald-500' : 'text-rose-500'}>%{formatMoney(c.change)}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto p-4 md:p-8 space-y-8">
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <h1 className="text-4xl font-black text-slate-900 leading-none tracking-tight text-slate-900">Finans <span className="text-blue-600 italic">Analiz</span></h1>
          </div>
          <div className="flex items-center gap-4 text-sm font-semibold bg-white px-4 py-2 rounded-2xl border border-slate-200 shadow-sm">
            <span className="flex items-center gap-2 text-slate-500">
              ...
              {lastUpdate ? `Veri: ${lastUpdate.toLocaleTimeString('tr-TR')}` : 'Yükleniyor...'}
            </span>
        ...
        {(errorStatus.news || errorStatus.fx) && (
          <div className="bg-amber-50 border border-amber-200 p-4 rounded-2xl flex items-center gap-3 text-amber-800 text-sm font-medium shadow-sm transition-all">
            <AlertCircle className="w-5 h-5 text-amber-600 flex-shrink-0" />
            <span>Bağlantı kısıtlamaları nedeniyle Vercel Blob Storage üzerinden yedek haberler yükleniyor.</span>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-8 space-y-8">
            ...
            <div className="bg-white rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-200 overflow-hidden">
              ...
              <div className="relative h-[400px] w-full bg-slate-50"><div id="tv_chart_container" className="h-full w-full" /></div>
            </div>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="text-[10px] uppercase text-slate-400 font-black tracking-widest border-b border-slate-50">
                      <th className="px-8 py-4">Enstrüman</th><th className="px-6 py-4">Fiyat</th><th className="px-6 py-4 text-center">Trend (24s)</th><th className="px-6 py-4 text-right">İşlem</th>
                    </tr>
                  </thead>
                    ...
                    {ASSETS.map(asset => {
                      ...
                      return (
                        <tr key={asset.code} onClick={() => setSelectedAsset(asset)} className={`group cursor-pointer transition-all ${isSelected ? 'bg-blue-50/50' : 'hover:bg-slate-50'}`}>
                          ...
                          <td className="px-6 py-5 w-32"><div className="h-8 w-full flex items-center"><Sparkline data={asset.type === 'currency' ? rates[asset.code]?.sparkline : commodities.find(c => c.code === asset.code)?.sparkline} color={isSelected ? "#2563eb" : "#94a3b8"} /></div></td>
            ...
            <div className="bg-white rounded-[2rem] p-8 border border-slate-200 shadow-xl shadow-slate-200/50 relative overflow-hidden">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-1 bg-blue-50 px-2 py-1 rounded-lg border border-blue-100 shadow-sm">
                  ...
                  <span className="text-[9px] font-bold text-blue-600 uppercase tracking-tighter italic">Vercel Blob Storage</span>
                </div>
              </div>
              <div className="space-y-5">
                {news.length > 0 ? news.map((item, i) => (
                  ...
                  <div className="py-8 text-center flex flex-col items-center gap-2 text-slate-400">
                    ...
                    <div className="text-xs italic tracking-tight">Haberler bulut yedeğinden yükleniyor...</div>
                  </div>
                )}
              </div>
              {lastNewsFetch && (
                <div className="mt-6 pt-4 border-t border-slate-50 text-[10px] text-slate-400 text-center italic font-medium">
                  Son Bulut Senkronizasyonu: {new Date(lastNewsFetch).toLocaleString('tr-TR')}
                </div>
              )}
            </div>
          </div>
      ...
      <footer className="text-center py-10 text-slate-400 text-xs px-4"><p>© 2026 Finans Merkezi - Vercel Blob Storage Caching v4.2</p></footer>
      <style>{`
